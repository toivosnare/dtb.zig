...
const Parser = struct {
    ...
    fn integer(comptime T: type, value: []const u8) !T {
        ...
        return std.mem.bigToNative(T, @as(*align(@alignOf(T)) const T, @ptrCast(@alignCast(value.ptr))).*);
    }
    ...
    fn integerList(self: *Parser, comptime T: type, value: []const u8) ![]T {
        ...
        while (i < list.len) : (i += 1) {
            list[i] = std.mem.bigToNative(T, @as(*align(@alignOf(T)) const T, @ptrCast(@alignCast(value[i * @sizeOf(T) ..].ptr))).*);
        }
        return list;
    }
...
fn cellsBigEndian(value: []const u8) ![]const u32 {
    ...
    return @as([*]align(@alignOf(u32)) const u32, @ptrCast(@alignCast(value)))[0 .. value.len / @sizeOf(u32)];
}
